...
function sizeof(o: Record<string, unknown>): number {
  ...
  Object.keys(o).forEach(key => {
    ...
    switch (typeof obj) {
      ...
      case 'object': if (obj) { bytes += sizeof(obj as Record<string, unknown>); } break;
    }
  });
  return bytes;
}
...
export function AddComment({ currentPeriod, project, onDismiss }: AddCommentProps) {
  ...
  const documentAuthorType = lists?.find(item => item.type === 'author' && item.name === 'Public');
  ...
  function deleteFile(doc: Document) {
    setDocuments(current => current.filter(item => item.documentFileName !== doc.documentFileName));
  }

  function learnMore() {
    setHasSeenCAC(true);
    track('Comment Modal CAC Learn More Clicked', { project_id: project?._id, project_name: project?.name });
    setCurrentPage(2);
  }
  ...
  function p2_becomeAMember() {
    track('Comment Modal Become Member Clicked', { project_id: project?._id, project_name: project?.name });
  ...
  async function p5_next() {
    ...
    setTotalSize(filesList.reduce((sum, file) => sum + file.size, sizeof(comment as unknown as Record<string, unknown>)));
    ...
    >
        ...
        {currentPage === 1 && (
          <div className="modal-content">
            <div className="modal-header">
              ...
              <button className="btn-close gtm-submit-comment_cancel-pg1" type="button" aria-label="Close" onClick={() => dismiss('dismissed page1')}></button>
            </div>

            <div className="modal-body">
              <section>
                <h2>How it Works</h2>
                <p>All accepted comments submitted to the Environmental Assessment Office (EAO) will be published online within seven days of receipt. Comments are not accepted if - in the EAO&apos;s view - they are profane, abusive or do not relate to the matter being consulted upon as stated in our  <a href="https://www2.gov.bc.ca/assets/gov/environment/natural-resource-stewardship/environmental-assessments/commenting-on-projects/public_comment_policy_v10.pdf" target="_blank">Public Comment Policy [PDF]</a>.</p>
                <p>All accepted comments are taken into consideration by EAO.</p>
                <p><strong>To ensure your comment meets EAO&apos;s requirements:</strong></p>
                <ul>
                  <li>Provide your location (City or Town)</li>
                  <li>Identify if you would like to remain anonymous or display your name publicly</li>
                  <li>Comments <strong><u>must not</u></strong> contain personal or otherwise identifying information of yourself or any other individuals <strong><u>in the text of the submission and/or attachments</u></strong> (e.g. home/work address, email address, telephone number)</li>
                  <li>Comments that contain profanity, abusive or intolerant language or does not relate to the matter being consulted upon, will not be accepted</li>
                  <li>Attachments must be no larger than 10 MB and must be a .png, .pdf, .gif, .jpg, .jpeg, .doc, .docx, .xls, .xlst, .ppt, .rtf, .pptx, .txt or .bmp file</li>
                </ul>
              </section>
              <section className="mb-0 mt-5">
                ...
                <p className="mb-0">Director, Digital Services<br />Environmental Assessment Office<br />836 Yates Street, Victoria, BC<br /><a href="mailto: EAO.EPICsystem@gov.bc.ca">EAO.EPICsystem@gov.bc.ca</a><br />(250) 356-7441</p>
              </section>
            </div>
            <div className="modal-footer">
              <div>
                <label className="control-label">
                  <input className="me-1" name="agreeConditions" type="checkbox" checked={agreeConditions} onChange={event => setAgreeConditions(event.target.checked)} />
                  I have read the above and understand what my submission should/shouldn&apos;t include
                </label>
              </div>
              <button type="button" className="btn btn-warning" disabled={!agreeConditions} onClick={p1_next}>Next</button>
            </div>
          </div>
        )}
      </form>
      ...
      <form onSubmit={event => event.preventDefault()}>
        {currentPage === 2 && (
          <div className="modal-content">
            ...
            <div className="modal-footer">
              <button className="btn btn-outline-white" type="button" onClick={() => setCurrentPage(page => page - 1)}>Back</button>
              <button className="btn btn-light" type="button" onClick={() => setCurrentPage(5)}>No Thanks</button>
              <button type="button" className="btn btn-warning" onClick={p2_becomeAMember}>Become a Member</button>
            </div>
          </div>
        )}
      </form>
      ...
      <form onSubmit={event => event.preventDefault()}>
        {currentPage === 3 && (
          <div className="modal-content">
            <div className="modal-header">
              <h4 className="modal-title">Community Advisory Committee</h4>
              <button className="btn-close gtm-submit-comment-cancel-pg2" type="button" aria-label="Close" onClick={() => dismiss('dismissed page2')}></button>
            </div>
            <div className="modal-body">
              <fieldset>
                ...
                <div className="mb-3 row">
                  <div className="mb-3">
                    <label htmlFor="nameInput" className="control-label">Full Name *</label>
                    <div className="form-control-container">
                      <input required className="form-control" type="text" name="nameInput" id="nameInput" value={nameInput} onChange={event => setNameInput(event.target.value)} />
                    </div>
                  </div>
                  <div className="mb-3">
                    <label htmlFor="caclocationInput" className="control-label">Location</label>
                    <div className="form-control-container">
                      <input className="form-control" type="text" name="caclocationInput" id="caclocationInput" value={caclocationInput} onChange={event => setCaclocationInput(event.target.value)} />
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-3 row">
                  <div className="mb-3">
                    ...
                    <div className="form-control-container">
                      <input required className="form-control" type="text" name="emailInput" id="emailInput" value={emailInput} onChange={event => setEmailInput(event.target.value)} />
                    </div>
                  </div>
                  <div className="mb-3">
                    <label htmlFor="emailConfirmInput" className="control-label">Confirm Email Address *</label>
                    <div className="form-control-container">
                      <input required className="form-control" type="text" name="emailConfirmInput" id="emailConfirmInput" value={emailConfirmInput} onChange={event => setEmailConfirmInput(event.target.value)} />
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-3">
                  ...
                  <div>
                    <p className="font-italic understandText">Please select all that apply. You may include additional details or information regarding your interest in the text box below. This information is for the EAO to better understand the membership of the committee, any information you provide here will not be considered as input into the assessment of the project. Please submit your comments on the proposed project through the public comment periods throughout the environmental assessment.</p>
                  </div>

                  {/* I live near */}
                  <div className="mb-3 row">
                    <label className="control-label"><input className="me-1" name="liveNear" type="checkbox" checked={liveNear} onChange={event => setLiveNear(event.target.checked)} />I live near or have particular knowledge of the project area</label>
                    {liveNear && (
                      <input className="form-control col-12" type="text" name="liveNearInput" id="liveNearInput" value={liveNearInput} onChange={event => setLiveNearInput(event.target.value)} />
                    )}
                  </div>
                  {/* I am a member of */}
                  <div className="mb-3 row">
                    <label className="control-label"><input className="me-1" name="memberOf" type="checkbox" checked={memberOf} onChange={event => setMemberOf(event.target.checked)} />I am a member of an organization with an interest in the project (provide organization name below)</label>
                    {memberOf && (
                      <input className="form-control col-12" type="text" name="memberOfInput" id="memberOfInput" value={memberOfInput} onChange={event => setMemberOfInput(event.target.value)} />
                    )}
                  </div>
                  {/* I have knowledge of */}
                  <div className="mb-3 row">
                    <label className="control-label"><input className="me-1" name="knowledgeOf" type="checkbox" checked={knowledgeOf} onChange={event => setKnowledgeOf(event.target.checked)} />I have particular knowledge of issues relevant to the potential project</label>
                    {knowledgeOf && (
                      <input className="form-control col-12" type="text" name="knowledgeOfInput" id="knowledgeOfInput" value={knowledgeOfInput} onChange={event => setKnowledgeOfInput(event.target.value)} />
                    )}
                  </div>
                  {/* Additional Notes */}
                  <div className="mb-3 row">
                    <label htmlFor="additionalNotesInput" className="control-label">Additional Notes:</label>
                    <textarea className="form-control" rows={4} name="additionalNotesInput" id="additionalNotesInput" value={additionalNotesInput} onChange={event => setAdditionalNotesInput(event.target.value)}></textarea>
                  </div>
                </div>

                <div className="mb-3">
                  <span className="control-label understandText">I understand that...</span>
                  <p className="font-italic understandText">Your personal information is collected by the Environmental Assessment Office for the purpose of sending you updates on {project.name} project under the authority of s.26(c) of the Freedom of Information and Protection of Privacy Act. In submitting your comment, you consent to receiving updates to the email address you have provided. This consent is valid from this date forward and may be revoked by contacting the EAO representative below. Should you have any questions about this collection, or wish to revoke your consent please contact:</p>
                  <ul className="font-italic understandText m-0">Director, Digital Services</ul>
                  <ul className="font-italic understandText m-0">Environmental Assessment Office</ul>
                  ...
                  <div className="form-control-container">
                    <label className="control-label">
                      <input required type="checkbox" aria-label="Checkbox for row item" name="acknowledged" checked={acknowledged} onChange={event => setAcknowledged(event.target.checked)} />
                      By checking this box, I acknowledge that I understand the above text.
                    </label>
                  </div>
                  <div className="form-control-container">
                    <label className="control-label">
                      <input required type="checkbox" aria-label="Checkbox for row item" name="termsOfReference" checked={termsOfReference} onChange={event => setTermsOfReference(event.target.checked)} />
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-outline-white" type="button" onClick={() => setCurrentPage(page => page - 1)}>Back</button>
              <button className="btn btn-warning" type="button" onClick={p3_next} disabled={!cacSignUpValid || emailInput !== emailConfirmInput}>Complete Submission</button>
            </div>
          </div>
        )}
      </form>
      ...
      {currentPage === 4 && (
        <div className="modal-content thank-you-content">
          <div className="modal-header">
            <button className="btn-close gtm-submit-comment-cancel-pg3" type="button" aria-label="Close" onClick={() => dismiss('dismissed page3')}></button>
          </div>
          <div className="modal-body pb-5">
            ...
            <p className="mb-4">We will be communicating with members of the Community Advisory Committee over the assessment stages of this project.  We will send out emails to members of this committee at key points through the assessment.</p>
            <p className="mb-4">Please add <a href={`mailto:${project.cacEmail}`}>{project.cacEmail}</a> to your contacts to ensure that our emails go to your inbox and are not filtered to your Spam folder.</p>
            <button className="btn btn-warning" type="button" onClick={() => setCurrentPage(page => page + 1)}>Continue to Commenting</button>
          </div>
        </div>
      )}
        ...
        {currentPage === 5 && (
          <div className="modal-content">
            ...
            <div className="modal-body">
              <fieldset>
                <div className="mb-3">
                  <label htmlFor="locationInput" className="control-label">Location *</label>
                  <input required className="form-control" type="text" name="locationInput" placeholder="IE: City, Province, etc."
                    id="locationInput" maxLength={50} value={locationInput} onChange={event => setLocationInput(event.target.value)} />
                </div>
                <div className="mb-3">
                  <label htmlFor="commentInput" className="control-label">Your Comment Submission*</label>
                  {commentTip && (
                  ...
                  <textarea className="form-control" rows={8} name="commentInput" id="commentInput" value={commentInput} onChange={event => setCommentInput(event.target.value)} placeholder="Comments must not contain personal or otherwise identifying information of yourself or any other individuals within your text submission and/or attachment (e.g. home/work address, email address, telephone number)."></textarea>
                </div>
                <FileUpload maxFiles={15} maxSize={10} showList={false} files={documents.map(doc => doc.upfile)} onFilesChange={addFiles} />
                {documents.length > 0 && (
                    ...
                    {documents.map(doc => (
                      <li key={doc.documentFileName}>
                        ...
                        <span className="cell actions">
                          <button className="btn btn-icon" type="button" title="Remove this file" onClick={() => deleteFile(doc)}>
                ...
                <div className="mb-3">
                  <span className="control-label">Name <em>(All comments are submitted anonymously by default.)</em></span>
                </div>
                <div className="mb-3">
                  <label className="control-label">
                    <input type="checkbox" aria-label="Checkbox for row item" name="anonymous" checked={makePublic} onChange={event => {
                  ...
                  <div>
                    <em>If you would like to add and display your name on the public website ensure you <u>select &quot;Please make my name visible to the public&quot;</u></em>
                  </div>
                </div>
                <div className="mb-3">
                  <div className="form-control-container">
                    <input required className="form-control" type="text" name="nameInput" maxLength={50} id="nameInput" value={contactName} onChange={event => setContactName(event.target.value)} disabled={!makePublic} />
                  </div>
                </div>
                {project.projectCAC && project.projectCACPublished && (
                  <div className="mb-3">
                    <label className="control-label">
                      <input type="checkbox" aria-label="Checkbox for row item" name="submittedCAC" checked={submittedCAC} onChange={event => setSubmittedCAC(event.target.checked)} />
            ...
            <div className="modal-footer">
              <button className="btn btn-outline-white" type="button" onClick={() => setCurrentPage(1)}>Back</button>
              {/* enable submission when required fields are entered and when either a file or comment is entered */}
              <button className="btn btn-warning" type="button" onClick={p5_next} disabled={!commentValid || (!commentInput && documents.length === 0) || submitting}>Submit</button>
            </div>
          </div>
        )}
      </form>
      ...
      {currentPage === 6 && (
        <div className="modal-content thank-you-content">
          ...
          <div className="modal-body pb-5">
            ...
            <p className="mb-4">Thank you for participating in the public comment period for the {project.name} Project.</p>
            <button className="btn btn-warning" type="button" onClick={() => dismiss('closed page4')}>Done</button>
          </div>
        </div>
      )}
    </dialog>
