...
describe('document search', () => {
  ...
  it('requests page 1 sorted by newest and renders the row', async () => {
    ...
    expect(lastDocumentRequest()).toBe(
      '/demi-search/search?dataset=Document&pageNum=0&pageSize=10&projectLegislation=default&sortBy=-datePosted&sortBy=&populate=true&fuzzy=false',
    );
  });
  ...
  it('restores currentPage, pageSize and sortBy from a deep link', async () => {
    ...
    await screen.findByText('Fish and Fish Habitat.pdf');
    expect(lastDocumentRequest()).toBe(
      '/demi-search/search?dataset=Document&pageNum=2&pageSize=25&projectLegislation=default&sortBy=+displayName&sortBy=&populate=true&fuzzy=false',
    );
  });
