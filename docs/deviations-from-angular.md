...
Where the React port behaves differently from the Angular app on purpose, and why. `e2e/support/helpers.ts` applies these to the request baseline, so a parity test that flags one of them is reading this list, not a bug.

One deviation runs through most of the `api:` entries below: **demi-search is the only backend.** The app makes no request to eagle-api at all — no `/api` prefix is built anywhere, and there is no fallback path to it. eagle-api still owns the data and pushes it into DEMI, and its Mongo `Config` document is still where the runtime values are edited, but the browser reads all of it from `/demi-search` and `/demi-projects`. An environment whose config document or rproxy is not ready for that serves the "temporarily unavailable" page rather than degrading to eagle-api.
...
- filters/search-filter-template: every keyword box searches as the user types — 300ms after the last keystroke, once the keyword is empty or two characters long. A shorter keyword searches as an empty one, so backspacing resets the results rather than leaving the last ones up. Angular only searched on Enter or the Search button.
- filters/search-filter-template: a searched-as-you-typed keyword records one `Search Executed` event, 1500ms after typing stops, instead of one per keystroke. Enter records its event straight away.
- filters/search-filter-template: a `Search Executed` event still waiting out its 1500ms pause is recorded at once when the box unmounts, so a reader who leaves the page or tab straight after typing is still counted. A search that had not gone out yet records nothing.
- filters/search-filter-template: there is no Search button. Angular had one beside every keyword box; here typing is the search, and Enter runs it without the pause. The box carries a screen-reader hint saying results update as you type, and the filter panel is never locked while a request is in flight.
...
- pages/content-search: the page is gone. Searching inside the documents is a scope of the documents tab — `/search?record=documents&scope=inside` — and `/search/content` redirects there with its keyword and paging. The old page's sort is dropped on the way: passages rank by relevance and the chunk index can sort by no field at all. `CONTENT_SEARCH` still gates the scope switch, so an environment with the flag off keeps only the names scope.
- pages/content-search: snippets are no longer written as HTML. The API's `<mark>` wrappers are stripped and the passage list marks the search terms itself, so `dangerouslySetInnerHTML` and the escape-then-re-open-`<mark>` guard both go away. A snippet whose source text held an escaped entity still reads it literally, as the Angular page did.
- pages/content-search: a passage carries no author. The Angular card showed a document's type, milestone and date; a chunk row carries no author field, so the meta line under the file name is date and type only.
- pages/project: the document-type tabs (Application, Certificate, Amendment(s)) are sub-tabs of Documents, joined by a new C&E Documents sub-tab. Old top-level paths redirect to the sub-tab, query string intact.
- pages/project: the project page has six sections (Overview, Updates, Engagement, Documents, Decisions, Compliance) as a `<nav>` of links rather than Angular's three ARIA tabs plus scattered document tabs. Updates and Documents show a count from a pageSize=1 search; Decisions and Compliance appear only when the project has a decision or the matching documents, so the pageSize=1 presence probes fire on every project page again (the same ones the Documents sub-tabs use; one query key, no duplicate fetch). The request baseline drops those probe lines; `documents-page.spec.tsx` and `project.spec.tsx` cover them. `/project-details` redirects to `/overview`, `/commenting` to `/engagement`.
- pages/project: the overview tab drops Angular's "Project ID" fact. No project carries `epicProjectID` on test or on prod, on either backend, so the row only ever rendered its dash. The model field stays: the project list filter still searches on it.
...
- pages/project: `/p/:projId/decisions` renders the EA decision and the certificate-set documents. The route exists on `develop` but not in the build deployed to test, where the URL falls through to the map page.
...
- pages/project: `decisions-tab` and `compliance-tab` are new. Angular's decisions template was entirely commented out and it had no compliance tab; both appear in the section nav only when the project has the matching data.
...
- pages/comments: the public cannot submit a comment here. ENGAGE owns comment submission now, so the page only reads the comment period and the comments already posted. The add-comment form, its CAC sign-up step, the comment POST and the attachment upload are all gone, along with the file-upload component the form was the only user of. The CAC unsubscribe page went with them: the last prod CAC sign-up was 2024-10-26, so no live emailed link is left to honour. To restore it, revert the commit that removed it and re-add `PUT /api/public/project/:id/cacRemoveMember` in eagle-api `api/controllers/cac.js`. The fields only that form read are dropped too: `commentTip` off the comment period, and `projectCAC`, `projectCACPublished` and `cacEmail` off the project.
...
- pages/comments: the period instructions render through `utils/safe-html`. Angular used `bypassSecurityTrustHtml` and a raw `[innerHTML]`.
...
- api: `List`, `Organization`, `RecentActivity` and `ProjectNotification` searches go to demi-search with `Project`, `Document` and `DocumentChunk`. Angular read all four from eagle-api. demi-search reads `top` bare, so it is sent as a plain parameter rather than `and[top]`; `companyType` goes as `and[companyType]`, which demi-search checks before the bare spelling. It 400s above 500 rows on a filtered search, so the proponent list is paged. The top-news strip and the proponent list no longer have an eagle-api-shaped variant: `/search` is the only shape sent.
- api: comment periods and comments come off demi-search too. Both are read through `/search`; Angular read them from `/commentperiod` and `/public/comment`. The period list and the single period filter under `and[project]` and `and[_id]`. `/search` ignores `fields=`, so the period list is trimmed in the API layer to the fields the engagement cards were always given; the full record carries an `additionalText` the cards never showed and demi-search does not store. The comment total comes from the envelope's `meta`.
- api/project: the single project record comes from DEMI's project document (`DEMI_PROJECTS_PATH`). DEMI stores the record under Track's field names, so `api/project.demiProjectToEagle` maps it back: `eagleId` is the Eagle `_id`, `projectType`/`projectState`/`address` are `type`/`status`/`location`, the GeoJSON `centroid` is unwrapped to the `[lon, lat]` pair the map takes, and `proponentId`/`proponentName` are rebuilt into the `{_id, name}` object the masthead and panel read. `commentPeriodForBanner` was an aggregation on eagle-api and has no DEMI counterpart, so it is derived from the project's comment periods using the same window the `cpStart`/`cpEnd` parameters applied — a period starting inside it, ending inside it, or spanning it. `metBannerImageUrl` was added to the period projection for that; the engagement cards ignore it. That read is the same query the engagement list runs (`['commentPeriods', projectId]`), so one project view asks for the periods once; it is not retried and a failure only costs the banner, never the project. Sharing it moved the list's normalizing — pulling the period name out of the instructions HTML and deduplicating by MET URL — off the cached rows and into a `select`, so the rows every other reader sees stay as the API sent them. A project DEMI does not hold renders the "Project not found" page; there is no second store to ask.
- api/project: DEMI has no `CELead`, `CELeadEmail`, `CELeadPhone`, `projectLeadId`, `responsibleEPDId`, `epicProjectID`, `commodity`, `fedElecDist`, `shortName`, `duration`, `primaryContact`, `proMember`, `isTermsAgreed`, `dateCommentsClosed`, `addedBy` or `updatedBy`, and withholds the `read`/`write`/`delete` ACLs by policy, so those facts render as "-". `dateUpdated` is no longer one of them: DEMI mirrors Eagle's own `dateUpdated` and the mapper reads it, so the overview tab's "Last updated" fills in. DEMI's `updatedAt` stamps its own last sync and stays ignored. `epicProjectID` is one of them, and no project on test carries it on the eagle-api side either, which is why the overview tab's "Project ID" fact is dropped outright rather than left to render a dash. `featuredDocuments` is not mapped at all: the featured-documents page runs its own document search on `isFeatured`, and nothing reads the project field. `applicableRegulation` and `phases` are mapped and are public in DEMI's field catalogue, but no project document on test carries either yet (60 sampled, 2026-09-08), so the decisions tab's Regulatory Transfer link and the assessment rail fall back to their own empty states there. `status` also changes vocabulary: eagle-api says "Complete" where DEMI's `projectState` says "Closed".
- api/project: the pinned Indigenous Nations come off that same DEMI project document rather than `/public/project/:id/pin`, so the project page fetches the document once for the record, the pins, the phase rail and the short link. DEMI stores the same `{_id, name, province}` rows and publishes them per project through `pinsRead[]`, but stores them unordered, so the `+name` sort and the paging the eagle-api route did happen in the API layer. A project document that carries no pins means the card is absent, which is the answer rather than a reason to ask somewhere else.
- api/getDocumentsByMultiId: the multi-id document read goes to demi-search. demi-search takes `docIds` as a bare parameter with the same pipe-separated list eagle-api's `/document?docIds=` route took — sent as `and[docIds]` it is dropped and the whole collection comes back. `/search` ignores `fields=`, so the row is trimmed in the API layer to the projection eagle-api was asked for, plus `_id`. demi-search's index has no `eaoStatus`, `internalOriginalName`, `labels`, `dateUploaded`, `dateReceived`, `documentFileSize`, `internalURL`, `internalMime`, `checkbox` or `documentAuthor`. Of those only `internalOriginalName` is read — it is the whole label of a comment attachment — so it falls back to `documentFileName`, then `displayName`, rather than rendering an empty row. The rest are unread on the comment attachment and related-document paths, and the download link needs only `_id`.
- pages/comments: a comment attachment and a comment period's related documents download through `utils/openDocumentDownload`, the same presigned path as every other document. Angular opened the eagle-api URL in a new tab, and the analytics event is now "Document Downloaded" rather than "Document Opened".
...
- utils/openDocumentDownload: a single download asks demi-api for a presigned URL and loads it in a hidden iframe, so the file arrives as an attachment and an error response cannot navigate the app away. Angular opened the eagle-api URL in a new tab. The anchor `href`, and the fallback for anything demi-api answers other than a URL, is now ``${SEARCH_API_PATH}/documents/:id/download?redirect=1`` — demi-api answers that with a 302 to the presigned object URL, so a plain navigation still downloads the file.
...
- components/download-panel: jobs live in memory only. Dismissing one that is still queued or running cancels it at demi-api, and leaving or reloading the site warns first, then cancels whatever is still being zipped. Nothing is left running for a reader who has gone.
...
- table/document rows: a click on a row selects it for download instead of downloading it. Angular downloaded the document on a click in any cell but Name, so a reader picking a row got a file. Now the row body is a second target for the checkbox and the Name link is the only thing that downloads. The keyboard goes through those two controls; the row itself is not a tab stop. The project documents, search results and project-notification document tables all follow it.
