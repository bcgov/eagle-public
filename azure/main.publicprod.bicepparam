...
//
// THIS DEPLOYMENT IS ON THE REQUEST PATH. It stopped being dark on 2026-08-21, when eao-nginx
// v2.7.14 repointed `location /` at the Front Door endpoint — so the storage account and the profile
// described here are what `projects.eao.gov.bc.ca` serves. An `az deployment group create` with this
// file is a production change. See the template header for the rollback.
