...
//
// WHY THIS EXISTS AT ALL. The hosting cutover happened on 2026-08-21 (eao-nginx v2.7.14), so rproxy
// now proxies `location /` to the Front Door endpoint. If Front Door or the storage account breaks,
// RPROXY IS STILL PERFECTLY HEALTHY: its readiness probe and `/nginx_status` stay green, because
// nginx is fine — it is the upstream that is gone. The whole public site can 502 with nothing in
// OpenShift noticing. Before the cutover that failure mode did not exist; the pods either served
// the app or failed their own readiness probe. The cutover created it, and this is what closes it.
//
// So this is no longer pre-positioning. The gap is open right now, and every hour this module is
// unapplied is an hour in which the only detection is a member of the public reporting it.
//
...
// also means the target did not have to change when the cutover moved what serves that name: the
// hostname is the contract, and OpenShift, Front Door or anything after them are implementation.
...
resource insights 'Microsoft.Insights/components@2020-02-02' = {
  ...
  properties: {
    ...
    // INGESTION IS PUBLIC, DELIBERATELY. This carried 'Disabled' when the module was written, on
    // the reasoning that no SDK reports here so any ingestion would be a surprise. That reasoning
    // had the writer backwards: the availability test's results ARE ingested telemetry, and the
    // thing writing them is Microsoft's probe fleet, reaching this component from the public
    // internet. 'Disabled' admits only networks joined to an Azure Monitor Private Link Scope, and
    // there is no AMPLS here — so it closes the one path the component exists to carry.
    //
    // What that would look like is the reason it matters. The failure is silent: an empty
    // availability blade, no error anywhere, and no alert to go missing because alerting is off
    // until an address is agreed. A blocked-ingestion component and a test that never deployed are
    // indistinguishable from the portal, which is exactly the "reads like a test that is not
    // running" state this module's header warns about.
    //
    // The exposure this gives back is junk telemetry from someone holding the connection string,
    // not disclosure — queries are separately governed by the line below, and the only data here is
    // whether a public website answered.
