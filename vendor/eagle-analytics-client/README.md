...
This is a vendored copy. `eagle-public` keeps it under `vendor/eagle-analytics-client` and depends on
it by path, so installing needs no registry and no token:

```json
"@digitalspace/eagle-analytics-client": "file:./vendor/eagle-analytics-client"
```

The source lives in the `eagle-analytics` repository under `client/`. The `dist` files here were
built from tag `client-v0.1.0`. To move to a newer version, build that tag in `eagle-analytics` and
replace `dist`, `package.json` and this file.
...
Building, testing and releasing happen in the `eagle-analytics` repository under `client/`. Nothing
here is built by `eagle-public`; the `dist` files are checked in as they came out of tag
`client-v0.1.0`.
