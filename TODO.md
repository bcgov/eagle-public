...
- project/project: a project the API cannot return renders a "Project not found" page instead of `alert("Uh-oh, couldn't load project")` followed by a redirect to `/projects`. The URL stays put, so the visitor can see which link failed. Reachable on test today: the list is served by demi-search, whose test corpus was seeded from prod, so 23 of its 359 projects have no record in the test eagle-api.
...
- `/api/config` slower than 5 s makes `loadConfig` fall back to `env.js` defaults (`SEARCH_API_PATH` empty, `ENVIRONMENT` dev) and the app renders with the wrong backend and banner. Seen twice under local load during parity runs. Angular has the same 5 s `AbortSignal.timeout`. Options: retry once, or block on config with a visible spinner instead of degrading silently. Needs a decision.
