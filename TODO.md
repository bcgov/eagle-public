...
- utils/openDocumentDownload: a single download asks demi-api for a presigned URL and navigates the current tab to it, so the file arrives as an attachment. Angular opened the eagle-api URL in a new tab; that stays the anchor `href` and the fallback whenever `SEARCH_API_PATH` is empty.
