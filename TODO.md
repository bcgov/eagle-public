...
- Project page redesign (PUBLIC-140): the in-EPIC comment period callout (`project.commentPeriodForBanner`, non-ENGAGE) left the project shell with the sidebar and comes back on the Overview tab, with its `Comment Period Banner Clicked` analytics event and tests. Until then only ENGAGE periods show a banner.
