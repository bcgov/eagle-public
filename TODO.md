...
- config: a failed `/api/config` is retried three times and then fatal, with an "EPIC is temporarily unavailable" page. Angular fell back to `env.js` after one 5 s timeout, which shipped `ACCESS_GATE` false and an empty `SEARCH_API_PATH`: the curtain opened and search hit the wrong backend.
